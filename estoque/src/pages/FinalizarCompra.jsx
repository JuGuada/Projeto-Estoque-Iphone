...
export default function FinalizarCompra() {
  ...
  async function copiarPixEConfirmar() {
    try { await navigator.clipboard.writeText(codigoPix); } catch { /* a confirmaÃ§Ã£o continua mesmo sem acesso Ã  Ã¡rea de transferÃªncia */ }
  ...
  async function consultarCep() {
    ...
    if (cep.length !== 8) return setErroCep("Digite um CEP com 8 nÃºmeros.");
    setBuscandoCep(true);
    setErroCep("");
    try {
      const response = await fetch(`https://viacep.com.br/ws/${cep}/json/`);
      const dados = await response.json();
      if (!response.ok || dados.erro) throw new Error("CEP nÃ£o encontrado.");
      ...
      setErroCep(error.message || "NÃ£o foi possÃ­vel consultar o CEP.");
    } finally {
  ...
  async function finalizar(event) {
    ...
    setErro("");
    if (!itens.length) return setErro("Seu carrinho estÃ¡ vazio.");
  ...
  if (!itens.length) {
    return <div className={styles.page}><UserHeader /><main className={styles.empty}><h1>Seu carrinho estÃ¡ vazio</h1><p>Adicione um produto antes de finalizar a compra.</p><button onClick={() => navigate("/")}>Voltar para a loja</button></main></div>;
  }
  ...
  return (
    <div className={styles.page}>
      ...
      <main className={styles.container}>
        <button className={styles.back} type="button" onClick={() => navigate("/carrinho")}>â† Voltar ao carrinho</button>
        ...
        <form className={styles.layout} onSubmit={finalizar}>
          <div className={styles.formColumn}>
            <section className={styles.card}>
              <div className={styles.sectionTitle}><span>01</span><div><h2>EndereÃ§o de entrega</h2><p>Informe onde vocÃª deseja receber o pedido.</p></div></div>
              {enderecoSalvo?.logradouro && <div className={styles.savedAddressSuggestion}><span className={styles.savedAddressIcon}>âŒ‚</span><div><small>ENDEREÃ‡O SALVO</small><strong>{enderecoSalvo.logradouro}, {enderecoSalvo.numero || "s/n"}</strong><p>{enderecoSalvo.bairro} Â· {enderecoSalvo.cidade}/{enderecoSalvo.estado} Â· CEP {enderecoSalvo.cep}</p></div><button type="button" onClick={() => { setEndereco({ cep: "", logradouro: "", numero: "", complemento: "", bairro: "", cidade: "", estado: "", ...enderecoSalvo }); setErroCep(""); }}>Usar este endereÃ§o</button></div>}
              <div className={styles.grid}>
                ...
                <label className={styles.wide}>EndereÃ§o<input required value={endereco.logradouro} onChange={alterar(setEndereco, "logradouro")} placeholder="Rua ou avenida" /></label>
                <label>NÃºmero<input required value={endereco.numero} onChange={alterar(setEndereco, "numero")} /></label>
            ...
            <section className={styles.card}>
              <div className={styles.sectionTitle}><span>02</span><div><h2>Pagamento</h2><p>Escolha a opÃ§Ã£o mais conveniente.</p></div></div>
              <div className={styles.paymentOptions}>
                {[['cartao','CartÃ£o de crÃ©dito','Em atÃ© 12x'],['pix','Pix','AprovaÃ§Ã£o imediata'],['boleto','Boleto','Vencimento em 2 dias']].map(([id,titulo,descricao]) => (
              ...
              {pagamento === "cartao" && <div className={styles.grid}>
                <label className={styles.wide}>Nome impresso no cartÃ£o<input value={cartao.nome} onChange={alterar(setCartao, "nome")} /></label>
                <label className={styles.wide}>NÃºmero do cartÃ£o<input inputMode="numeric" maxLength="19" value={cartao.numero} onChange={alterar(setCartao, "numero")} placeholder="0000 0000 0000 0000" /></label>
              ...
              {pagamento === "pix" && <div className={styles.notice}>Ao confirmar, o QR Code Pix serÃ¡ gerado em uma janela segura. Desconto de 3% jÃ¡ aplicado.</div>}
              {pagamento === "boleto" && <div className={styles.notice}>O boleto serÃ¡ gerado apÃ³s a confirmaÃ§Ã£o e enviado ao seu e-mail.</div>}
            </section>
          </div>

          <aside className={styles.summaryCard}>
            <h2>Resumo do pedido</h2>
            <div className={styles.items}>{itens.map((item) => <div className={styles.item} key={`${item.categoria}-${item.id}`}><img src={item.imagem || "/imagens/produto.png"} alt="" /><div><strong>{item.nome}</strong><span>{item.cor ? `${item.cor} Â· ` : ""}{item.quantidade || 1} unidade(s)</span></div><b>{formatarPrecoProduto(converterPrecoParaNumero(item.preco) * Number(item.quantidade || 1))}</b></div>)}</div>
            ...
            <div className={styles.totals}><p><span>Subtotal</span><strong>{formatarPrecoProduto(subtotal)}</strong></p><p><span>Frete</span><strong>{frete === 0 ? "GrÃ¡tis" : formatarPrecoProduto(frete)}</strong></p>{descontoCupom > 0 && <p className={styles.discount}><span>Cupom (10%)</span><strong>âˆ’ {formatarPrecoProduto(descontoCupom)}</strong></p>}{descontoPix > 0 && <p className={styles.discount}><span>Desconto Pix (3%)</span><strong>âˆ’ {formatarPrecoProduto(descontoPix)}</strong></p>}<p className={styles.grandTotal}><span>Total</span><strong>{formatarPrecoProduto(total)}</strong></p></div>
            <label className={styles.terms}><input type="checkbox" checked={aceitou} onChange={(e) => setAceitou(e.target.checked)} /><span>Confirmo que os dados estÃ£o corretos e aceito os termos da compra.</span></label>
            {erro && <p className={styles.error} role="alert">{erro}</p>}
            <button className={styles.finish} type="submit" disabled={processando}>{processando ? "Processando pedido..." : `Confirmar â€¢ ${formatarPrecoProduto(total)}`}</button>
            <small className={styles.security}>Compra protegida e dados enviados com seguranÃ§a.</small>
          </aside>
        </form>
      </main>
      {etapaPix && <div className={styles.pixOverlay} role="dialog" aria-modal="true" aria-label="Pagamento via Pix"><section className={styles.pixModal}>
        ...
        {etapaPix === "qrcode" && <><div className={styles.pixModalHeader}><span>PIX Â· PAGAMENTO SEGURO</span><h2>Escaneie o QR Code</h2><p>Abra o aplicativo do seu banco ou copie o cÃ³digo abaixo.</p></div><div className={styles.pixModalBody}><div className={styles.qrCode} aria-label="QR Code Pix ilustrativo"><svg viewBox="0 0 120 120" role="img"><rect width="120" height="120" fill="white"/><path d="M6 6h34v34H6zm7 7v20h20V13zM80 6h34v34H80zm7 7v20h20V13zM6 80h34v34H6zm7 7v20h20V87zM50 8h8v8h-8zm12 0h8v16h-8zM48 26h10v10H48zm17 4h8v14h-8zM48 48h12v8H48zm18 0h8v18h-8zm14 0h9v9h-9zm14 0h18v8H94zM50 62h8v17h-8zm12 10h16v8H62zm22-10h8v18h-8zm14 0h14v9H98zM47 87h10v10H47zm15-3h9v20h-9zm14 1h12v8H76zm16-5h20v9H92zm-14 20h9v14h-9zm14-4h8v8h-8zm12 10h10v8h-10z" fill="#111"/></svg></div><strong className={styles.pixValue}>{formatarPrecoProduto(total)}</strong><span className={styles.pixDiscount}>VocÃª economizou {formatarPrecoProduto(descontoPix)} com o Pix</span><code className={styles.pixCode}>{codigoPix}</code><button type="button" className={styles.copyPix} onClick={copiarPixEConfirmar} disabled={processando}>{processando ? "Confirmando pagamento..." : "Copiar cÃ³digo Pix"}</button><button type="button" className={styles.closePix} onClick={() => setEtapaPix(null)} disabled={processando}>Voltar ao checkout</button><small>Ambiente ilustrativo: copiar o cÃ³digo confirma o pagamento.</small></div></>}
        {etapaPix === "confirmado" && <div className={styles.pixConfirmed}><span>âœ“</span><h2>Pedido confirmado!</h2><p>Pagamento Pix identificado com sucesso.</p><small>VocÃª serÃ¡ direcionado para acompanhar o pedido.</small></div>}
      </section></div>}
    </div>
  );
}
