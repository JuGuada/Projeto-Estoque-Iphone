...
export default function Perfil() {
    ...
    const [nome, setNome] = useState(usuario?.nome || "UsuÃ¡rio");
    ...
    function salvarEndereco(event) {
        ...
        setMensagem("EndereÃ§o salvo com sucesso.");
        setErro("");
    ...
    async function salvarPerfil(event) {
        event.preventDefault();
        if (!usuario?.id) return setErro("NÃ£o foi possÃ­vel identificar sua conta.");
    ...
    async function alterarSenha(event) {
        ...
        if (senhas.nova !== senhas.confirmar) return setErro("A confirmaÃ§Ã£o da senha nÃ£o confere.");
        setSalvando(true);
        try {
    ...
    return <div className={styles.page}><div className={styles.main}><UserHeader /><main className={styles.content}>
        <header className={styles.heading}><h1>Perfil</h1><p>Gerencie suas informaÃ§Ãµes pessoais e a seguranÃ§a da conta.</p></header>
        {(mensagem || erro) && <p className={erro ? styles.profileError : styles.profileSuccess} role="status">{erro || mensagem}</p>}
        <section className={styles.card}><div className={styles.cardHeader}><h2>InformaÃ§Ãµes pessoais</h2><button type="button" className={styles.editButton} onClick={() => setEditando((atual) => !atual)}>{editando ? "Cancelar" : "Editar perfil"}</button></div><div className={styles.profileBody}>
            ...
            {editando ? <form className={styles.fields} onSubmit={salvarPerfil}><label>Nome<input required value={nome} onChange={(event) => setNome(event.target.value)} /></label><label>E-mail<input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label><button disabled={salvando} type="submit" className={styles.saveButton}>{salvando ? "Salvando..." : "Salvar alteraÃ§Ãµes"}</button></form> : <div className={styles.fields}><div><span>Nome</span><strong>{nome}</strong></div><div><span>E-mail</span><strong>{email}</strong></div></div>}
        </div></section>
        <section className={styles.addressCard}>
            <div className={styles.addressHeader}><div><h2>EndereÃ§o salvo</h2><p>Use este endereÃ§o para preencher suas prÃ³ximas compras.</p></div><button type="button" className={styles.editButton} onClick={() => setEditandoEndereco((atual) => !atual)}>{editandoEndereco ? "Cancelar" : endereco.logradouro ? "Editar endereÃ§o" : "Adicionar endereÃ§o"}</button></div>
            {editandoEndereco ? <form className={styles.addressForm} onSubmit={salvarEndereco}>
                <label>CEP<input required value={endereco.cep} onChange={alterarEndereco("cep")} placeholder="00000-000" maxLength="9" /></label><label className={styles.addressWide}>EndereÃ§o<input required value={endereco.logradouro} onChange={alterarEndereco("logradouro")} placeholder="Rua ou avenida" /></label><label>NÃºmero<input required value={endereco.numero} onChange={alterarEndereco("numero")} /></label><label>Complemento<input value={endereco.complemento} onChange={alterarEndereco("complemento")} placeholder="Opcional" /></label><label>Bairro<input required value={endereco.bairro} onChange={alterarEndereco("bairro")} /></label><label>Cidade<input required value={endereco.cidade} onChange={alterarEndereco("cidade")} /></label><label>Estado<input required value={endereco.estado} onChange={alterarEndereco("estado")} maxLength="2" placeholder="SP" /></label><button type="submit" className={styles.addressSave}>Salvar endereÃ§o</button>
            </form> : endereco.logradouro ? <div className={styles.savedAddress}><span className={styles.addressIcon}>âŒ‚</span><div><strong>{endereco.logradouro}, {endereco.numero || "s/n"}</strong>{endereco.complemento && <small>{endereco.complemento}</small>}<p>{endereco.bairro} Â· {endereco.cidade}/{endereco.estado}</p><small>CEP {endereco.cep}</small></div></div> : <div className={styles.emptyAddress}><span>âŒ‚</span><div><strong>Nenhum endereÃ§o salvo</strong><p>Adicione um endereÃ§o para agilizar sua prÃ³xima compra.</p></div></div>}
        </section>
        <section className={styles.cardSecurity}><h2>SeguranÃ§a da conta</h2><button type="button" className={styles.securityRow} onClick={() => setSenhaAberta((atual) => !atual)}><span className={styles.securityIcon}>â€¢â€¢â€¢</span><span><strong>Alterar senha</strong><small>Confirme sua senha atual antes de cadastrar uma nova.</small></span><span className={styles.arrowRight}>â€º</span></button>
