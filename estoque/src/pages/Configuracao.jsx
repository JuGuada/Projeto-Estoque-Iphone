...
const permissoes = [
  { id: 1, nome: 'VisÃ£o geral', desc: 'Visualizar indicadores e resumo da loja' },
  { id: 2, nome: 'Visualizar estoque', desc: 'Consultar produtos e quantidades' },
  { id: 3, nome: 'Editar produtos e preÃ§os', desc: 'Alterar informaÃ§Ãµes e valores' },
  { id: 4, nome: 'MovimentaÃ§Ãµes', desc: 'Registrar entradas, saÃ­das e vendas' },
  { id: 5, nome: 'Cadastrar produtos', desc: 'Adicionar novos itens ao catÃ¡logo' },
  { id: 6, nome: 'Gerenciar usuÃ¡rios', desc: 'Criar contas e definir cargos' },
  { id: 7, nome: 'Alterar configuraÃ§Ãµes', desc: 'Modificar permissÃµes, regras e notificaÃ§Ãµes' },
];
...
const resumo = (ids = []) => {
  if (!ids.length) return 'Nenhuma Ã¡rea liberada';
  if (ids.length === permissoes.length) return 'Acesso completo a todas as Ã¡reas';
...
export default function Configuracao() {
  ...
  async function salvar() { if (perfil === 'Administrador') return setMensagem('O administrador sempre possui acesso total.'); try { await apiRequest(`/permissoes/${perfil.toLowerCase()}`, { method: 'PUT', body: JSON.stringify({ permissoes: acessos[perfil] }) }); setMensagem('PermissÃµes salvas com sucesso.'); } catch (e) { setMensagem(e.message); } }
  async function criar(e) { e.preventDefault(); setErroModal(''); if (!form.nome.trim() || !form.email.trim()) return setErroModal('Preencha nome e e-mail.'); if (form.senha.length < 6) return setErroModal('A senha inicial precisa ter pelo menos 6 caracteres.'); setCriando(true); try { const tipo = form.cargo === 'Administrador' ? 'admin' : form.cargo.toLowerCase(); const criado = await apiRequest('/usuarios', { method: 'POST', body: JSON.stringify({ nome: form.nome.trim(), email: form.email.trim(), senha: form.senha, tipo }) }); setMembros((a) => [...a, { ...criado, cargo: form.cargo }]); setForm({ nome: '', email: '', senha: '', cargo: 'Vendedor' }); setModal(false); setMensagem('UsuÃ¡rio criado com sucesso.'); } catch (e) { setErroModal(e.message || 'NÃ£o foi possÃ­vel criar o usuÃ¡rio.'); } finally { setCriando(false); } }
  async function remover(m) { if (m.tipo === 'admin') return setMensagem('A conta administradora nÃ£o pode ser removida.'); try { await apiRequest(`/usuarios/${m.id}`, { method: 'DELETE' }); setMembros((a) => a.filter((x) => x.id !== m.id)); } catch (e) { setMensagem(e.message); } }

  return <AdminLayout><div className={styles.container}>
    <div className={styles.mainHeader}><h1>ConfiguraÃ§Ãµes</h1><p>Gerencie equipe, acessos e regras do sistema</p></div>
    <div className={styles.segmentedControl}>{[['equipe', 'Equipe e permissÃµes'], ['regras', 'Regras de negÃ³cio'], ['notificacoes', 'NotificaÃ§Ãµes']].map(([id, n]) => <button type="button" key={id} className={`${styles.segmentBtn} ${aba === id ? styles.segmentActive : ''}`} onClick={() => setAba(id)}>{n}</button>)}</div>
    {mensagem && <p className={styles.feedback} role="status">{mensagem}</p>}
    {aba === 'equipe' && <div className={styles.fadeContainer}>
      <div className={styles.card}><div className={styles.cardHeader}><div className={styles.cardTitleGroup}><img src="/imagens/equipe.png" alt="" /><h3>Membros da equipe</h3></div><button type="button" className={styles.btnInvite} onClick={() => { setErroModal(''); setModal(true); }}>+ Criar usuÃ¡rio</button></div>
        <div className={styles.memberList}>{membros.map((m) => <div key={m.id} className={styles.memberItem}><div className={styles.memberInfo}><div className={styles.avatar} style={{ backgroundColor: corDoNome(m.nome) }}>{iniciais(m.nome)}</div><div><p className={styles.memberName}>{m.nome}</p><p className={styles.memberEmail}>{m.email}</p></div></div><div className={styles.memberActions}><span className={styles.cargoTag}>{m.cargo}</span>{m.tipo !== 'admin' && <button type="button" className={styles.btnDelete} onClick={() => remover(m)} title="Remover usuÃ¡rio">Ã—</button>}</div></div>)}</div>
      </div>
      <h2 className={styles.sectionTitle}>UsuÃ¡rios e permissÃµes</h2><p className={styles.sectionSubtitle}>Escolha um cargo e determine exatamente quais Ã¡reas ele pode acessar.</p>
      ...
      <p className={styles.smallSectionTitle}>PERMISSÃ•ES PARA: <strong>{perfil.toUpperCase()}</strong></p>
