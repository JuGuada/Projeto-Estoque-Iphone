...
function normalizarMovimentacao(item, operadorAtual = 'Administrador') {
  return {
    ...
    sku: item.sku || item.numero_serie || 'SKU não informado',
...
export default function Movimentacoes() {
  ...
  const handleRegistrar = (e) => {
    ...
    if (!produtoSelecionado) {
      setErro('Selecione um produto para registrar a movimentação.');
      return;
    }
    ...
    fetch(`${API_URL}/movimentacoes`, {
      ...
      .then(async (response) => {
        ...
        if (!response.ok) throw new Error(data.erro || 'Erro ao registrar movimentação.');
  ...
  const unidadesSaida = historico.filter((h) => h.tipo === 'Saída').reduce((acc, item) => acc + item.qtd, 0);

  const historicoFiltrado = historico.filter((item) => {
    if (filtro === 'Entradas') return item.tipo === 'Entrada';
    if (filtro === 'Saídas') return item.tipo === 'Saída';
    return true;
  }).filter((item) => {
  ...
  return (
    <AdminLayout>
      ...
      {/* ... (todo o resto do JSX permanece igual, incluindo banners, métricas e formulário) ... */}
      ...
      <div className={styles.bannersSection}>
        <div className={`${styles.bannerCard} ${styles.bannerPurple} ${styles.anim1}`}>
          <div className={styles.badgeBannerFull}>O campeão de bilheteria</div>
          ...
          <p className={styles.bannerSubtitle}>
            Saindo mais rápido do que<br />estoque em Black Friday
          </p>
        </div>
        ...
        <div className={`${styles.bannerCard} ${styles.bannerCyan} ${styles.anim2}`}>
          ...
          <div className={styles.centerCyanContent}>
            ...
            <p className={styles.bannerSubtitle}>A equipe tá voando<br />no atendimento</p>
          </div>
        </div>

        <div className={`${styles.bannerCard} ${styles.bannerImage} ${styles.anim3}`}>
          <div className={styles.textOverlay}>
            <p>Especialistas dedicados a garantir a melhor experiência para o cliente.</p>
          </div>
        </div>
      </div>
      ...
      <div className={styles.metricsGrid}>
        ...
        <div className={styles.metricCard}>
          ...
          <span className={styles.metricLabel}>Unidades em saída</span>
        </div>
      </div>
        ...
        <div className={styles.cardForm}>
          ...
          <form onSubmit={handleRegistrar} className={styles.form}>
            ...
            <div className={styles.tipoButtons}>
              <button type="button" className={`${styles.btnTipo} ${tipoMovimentacao === 'Saida' ? styles.btnSaidaActive : ''}`} onClick={() => setTipoMovimentacao('Saida')}>Saída / Venda</button>
            ...
            <div className={styles.inputGroup}>
              ...
              <select value={responsavel} onChange={(e) => setResponsavel(e.target.value)} className={styles.input} aria-label="Selecionar operador administrativo">
                ...
                {operadores.map((item) => <option key={item.id} value={item.nome || item.email}>{item.nome || item.email}{item.email && item.nome ? ` — ${item.email}` : ''}</option>)}
              </select>
            </div>

            <button type="submit" className={styles.btnSubmit}>+ Registrar movimentação</button>
          ...
          <div className={styles.historyHeader}>
            ...
            <div className={styles.historyFilters}>
              <div className={styles.calendarArea} ref={calendarioRef}>
                ...
                {dataFiltro && <button type="button" className={styles.clearDate} onClick={() => { setDataFiltro(''); setDataTemporaria(''); }} aria-label="Limpar filtro de data">×</button>}
                {calendarioAberto && <div className={styles.calendarPopup}>
                  <div className={styles.calendarHeader}><button type="button" onClick={() => setMesCalendario((mes) => new Date(mes.getFullYear(), mes.getMonth() - 1, 1))}>‹</button><strong>{mesCalendario.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' })}</strong><button type="button" onClick={() => setMesCalendario((mes) => new Date(mes.getFullYear(), mes.getMonth() + 1, 1))}>›</button></div>
            ...
            <table className={styles.table}>
              <thead>
                <tr>
                  ...
                  <th style={{ textAlign: 'right' }}>RESPONSÁVEL</th>
                </tr>
              </thead>
              <tbody>
                {carregando && (
                ...
                {!carregando && historicoFiltrado.length === 0 && (
                  <tr><td colSpan="6" className={styles.emptyHistory}>Nenhuma movimentação encontrada para este período.</td></tr>
                )}
              </tbody>
            </table>
          </div>
