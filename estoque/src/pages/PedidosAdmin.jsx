...
const STATUS = ["Pedido realizado", "Em separaÃ§Ã£o", "Preparando envio", "Em transporte", "Entregue", "Cancelado"];
const statusAtual = (status) => status === "Confirmado" ? "Preparando envio" : ["ConcluÃ­do", "Concluido"].includes(status) ? "Entregue" : status || "Pedido realizado";
const data = (valor) => valor ? new Date(valor).toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" }) : "â€”";
const nomePagamento = (tipo) => ({ cartao: "CartÃ£o de crÃ©dito", pix: "Pix", boleto: "Boleto" }[tipo] || tipo || "NÃ£o informado");
...
export default function PedidosAdmin() {
  ...
  return <div className={styles.page}>
    <header className={styles.header}><div><span>AdministraÃ§Ã£o</span><h1>Pedidos</h1><p>Controle clientes, pagamentos e entregas em um sÃ³ lugar.</p></div></header>
    <section className={styles.stats}>
      ...
      <article><span>EM LOGÃSTICA</span><strong>{resumo.logistica}</strong><small>preparo ou transporte</small></article>
    ...
    {selecionado && <div className={styles.overlay} onMouseDown={() => setSelecionado(null)}><section className={styles.modal} onMouseDown={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
      <header className={styles.modalHeader}><div><span>DETALHES DO PEDIDO</span><h2>Pedido #{selecionado.id}</h2><p>Realizado em {data(selecionado.criadoEm)}</p></div><button onClick={() => setSelecionado(null)} aria-label="Fechar">Ã—</button></header>
      <div className={styles.infoGrid}><article><span>CLIENTE</span><strong>{selecionado.usuarioNome || "Cliente"}</strong><small>{selecionado.usuarioEmail || "E-mail nÃ£o informado"}</small><small>ID: {selecionado.usuarioId}</small></article><article><span>PAGAMENTO</span><strong>{nomePagamento(selecionado.formaPagamento)}</strong><small>{selecionado.parcelas > 1 ? `${selecionado.parcelas} parcelas` : "Pagamento Ã  vista"}</small></article><article><span>ENTREGA</span><strong>{selecionado.endereco?.logradouro ? `${selecionado.endereco.logradouro}, ${selecionado.endereco.numero || "s/n"}` : "NÃ£o informado"}</strong><small>{selecionado.endereco?.bairro} Â· {selecionado.endereco?.cidade}/{selecionado.endereco?.estado}</small><small>CEP {selecionado.endereco?.cep || "â€”"}</small></article></div>
      <div className={styles.statusControl}><label>Status do pedido<select value={statusAtual(selecionado.status)} disabled={salvando === selecionado.id} onChange={(e) => mudarStatus(selecionado, e.target.value)}>{STATUS.map((item) => <option key={item}>{item}</option>)}</select></label><small>{salvando === selecionado.id ? "Salvando alteraÃ§Ã£o..." : "O cliente verÃ¡ esta atualizaÃ§Ã£o em Meus pedidos."}</small></div>
      ...
      <footer className={styles.totals}><p><span>Subtotal</span><b>{formatarPrecoProduto(selecionado.subtotal)}</b></p><p><span>Frete</span><b>{Number(selecionado.frete) ? formatarPrecoProduto(selecionado.frete) : "GrÃ¡tis"}</b></p>{Number(selecionado.desconto) > 0 && <p><span>Descontos{selecionado.cupom ? ` Â· cupom ${selecionado.cupom}` : ""}</span><b>âˆ’ {formatarPrecoProduto(selecionado.desconto)}</b></p>}<p><span>Total</span><strong>{formatarPrecoProduto(selecionado.total)}</strong></p></footer>
    </section></div>}
  </div>;
}
