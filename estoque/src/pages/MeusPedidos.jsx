...
const etapasPedido = ["Pedido realizado", "Em separação", "Preparando envio", "Em transporte", "Entregue"];
  ...
  return <div className={styles.tracking} aria-label={`Acompanhamento do pedido: ${etapasPedido[atual]}`}>
    {etapasPedido.map((etapa, indice) => {
      ...
      return <div className={`${styles.trackingStep} ${concluida ? styles.trackingDone : ""} ${ativa ? styles.trackingActive : ""}`} key={etapa}>
        <span className={styles.trackingIcon} aria-hidden="true">{concluida ? "✓" : ativa ? "●" : "○"}</span>
...
export default function MeusPedidos() {
  ...
  return <div className={styles.page}><div className={styles.main}><UserHeader /><main className={styles.content}>
    ...
    <div className={styles.ordersList}>{filtrados.length ? filtrados.map((pedido) => {
      ...
      return <section className={`${styles.orderCard} ${styles.clickableCard}`} key={pedido.id} onClick={() => setSelecionado(pedido)}>
        <div className={styles.orderHeader}><div><h2>{rascunho ? "Compra não finalizada" : `Pedido #${numeroPedido(pedido)}`}</h2><p>Iniciado em {formatarData(pedido.criadoEm)}</p></div><span className={`${styles.status} ${rascunho ? styles.statusPending : ""} ${status === "Cancelado" ? styles.statusCanceled : status === "Entregue" ? styles.statusDone : ""}`}>{status}</span></div>
        {rascunho ? <div className={styles.finishNotice}><strong>Finalize o seu pedido</strong><span>O pagamento ainda não foi realizado. Continue a compra para confirmar o pedido e iniciar o acompanhamento da entrega.</span></div> : <ProgressoPedido status={pedido.status} />}
        ...
        <footer className={styles.orderFooter}><p>{status === "Entregue" ? <>Detalhes e comprovante da compra disponíveis<br /><b>{usuario?.email || "E-mail não informado"}</b></> : status === "Cancelado" ? "Pedido cancelado. Nenhuma alteração foi feita no estoque." : rascunho ? "Finalize a compra para transformar este rascunho em pedido." : `Acompanhe aqui cada etapa até a entrega do seu pedido.`}</p>{rascunho && <div className={styles.orderActions}><button type="button" onClick={(e) => { e.stopPropagation(); cancelar(pedido); }}>Cancelar</button><button type="button" className={styles.primaryAction} onClick={(e) => { e.stopPropagation(); retomar(pedido); }}>Finalizar pedido</button></div>}</footer>
      </section>;
  ...
  {selecionado && <div className={styles.detailsOverlay} onMouseDown={() => setSelecionado(null)}><section className={styles.detailsModal} role="dialog" aria-modal="true" aria-labelledby="titulo-detalhes" onMouseDown={(e) => e.stopPropagation()}>
    <header className={styles.detailsHeader}><div><span>DETALHES DA COMPRA</span><h2 id="titulo-detalhes">{pedidoEmRascunho(selecionado) ? "Compra não finalizada" : `Pedido #${numeroPedido(selecionado)}`}</h2><p>{formatarData(selecionado.criadoEm)}</p></div><button type="button" onClick={() => setSelecionado(null)} aria-label="Fechar">×</button></header>
    <div className={styles.detailsStatus}><span>Status atual</span><strong>{statusDoPedido(selecionado)}</strong></div>
    {pedidoEmRascunho(selecionado) ? <div className={styles.detailsFinishNotice}><strong>Finalize o seu pedido</strong><span>O pagamento ainda está pendente e o acompanhamento começará após a confirmação.</span></div> : <div className={styles.detailsTracking}><ProgressoPedido status={selecionado.status} /></div>}
    <div className={styles.detailsItems}>{(selecionado.itens || []).map((item) => <div key={`${selecionado.id}-detalhe-${item.id}`}><img src={item.imagem || "/imagens/produto.png"} alt={item.nome} /><span><strong>{item.nome}</strong><small>{item.cor || item.modelo || "Produto"} • {item.quantidade || 1} unidade(s)</small></span><b>{formatarPrecoProduto(converterPrecoParaNumero(item.preco) * Number(item.quantidade || 1))}</b></div>)}</div>
    <div className={styles.detailsGrid}><div><span>Forma de pagamento</span><strong>{selecionado.formaPagamento || "Ainda não definida"}{selecionado.parcelas > 1 ? ` • ${selecionado.parcelas}x` : ""}</strong></div><div><span>Cliente</span><strong>{selecionado.usuarioNome || usuario?.nome || "Cliente"}</strong><small>{selecionado.usuarioEmail || usuario?.email || "E-mail não informado"}</small></div><div className={styles.detailsAddress}><span>Endereço de entrega</span><strong>{selecionado.endereco?.logradouro ? `${selecionado.endereco.logradouro}, ${selecionado.endereco.numero || "s/n"}` : "Ainda não informado"}</strong><small>{selecionado.endereco?.bairro ? `${selecionado.endereco.bairro} • ${selecionado.endereco.cidade}/${selecionado.endereco.estado} • CEP ${selecionado.endereco.cep}` : "Complete o endereço ao finalizar a compra"}</small></div></div>
    <div className={styles.detailsTotals}><p><span>Subtotal</span><strong>{formatarPrecoProduto(selecionado.subtotal)}</strong></p><p><span>Frete</span><strong>{Number(selecionado.frete || 0) === 0 ? "Grátis" : formatarPrecoProduto(selecionado.frete)}</strong></p><p><span>Total da compra</span><strong>{formatarPrecoProduto(selecionado.total)}</strong></p></div>
  </section></div>}
  </div>;
}
